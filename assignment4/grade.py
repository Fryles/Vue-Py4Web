...
class ProtoAssignment(py4web):
    ...
    def __init__(self, app_path, debug=False):
        ...
        self.test_images = [
            os.path.join("/tmp/test_images", f)
...
class Assignment(ProtoAssignment):
    ...
    def step1(self):
        ...
        self.goto("index")
        assert (
            len(self.get_contacts()) == 0
        ), "S1-1 There should be no contacts initially."
        ...
        assert (
            ...
            .get_attribute("value")
            == ""
        ), "S1-3 The name is initially not empty."
        assert (
            cs[0]
            .find_element(By.CSS_SELECTOR, "input[name='affiliation']")
            .get_attribute("value")
            == ""
        ), "S1-4 The affiliation is initially not empty."
        assert (
            cs[0]
            .find_element(By.CSS_SELECTOR, "textarea[name='description']")
            .get_attribute("value")
            == ""
        ), "S1-5 The descripton is initially not empty."
    ...
    def step2(self):
        ...
        assert i_name.get_attribute(
            "readonly"
        ), "S2-1 The name should be readonly initially."
        assert i_aff.get_attribute(
            "readonly"
        ), "S2-2 The affiliation should be readonly initially."
        # Name.
        i_name.click()
        assert not i_name.get_attribute(
            "readonly"
        ), "S2-3 The name should be editable after clicking."
        self.name = str(uuid.uuid4())
        self.aff = str(uuid.uuid4())
        i_name.send_keys(self.name)
        title.click()
        assert (
            i_name.get_attribute("value") == self.name
        ), "S2-4 The name should be editable."
        assert i_name.get_attribute(
            "readonly"
        ), "S2-5 The name should be readonly after losing focus."
        # Affiliation.
        i_aff.click()
        assert not i_aff.get_attribute(
            "readonly"
        ), "S2-6 The affiliation should be editable after clicking."
        i_aff.send_keys(self.aff)
        title.click()
        assert (
            i_aff.get_attribute("value") == self.aff
        ), "S2-7 The affiliation should be preserved."
        assert i_aff.get_attribute(
            "readonly"
        ), "S2-8 The affiliation should be readonly after losing focus."
        # Sanity check.
        assert (
            i_name.get_attribute("value") == self.name
        ), "S2-9 The name should be again preserved."
        return (
            1,
            "Name and affiliation change to editable when clicked, and to read-only when they lose focus.",
        )
    ...
    def step3(self):
        ...
        assert i_descr.get_attribute(
            "readonly"
        ), "S3-1 The description should be readonly initially."
        i_descr.click()
        assert not i_descr.get_attribute(
            "readonly"
        ), "S3-2 The description should be editable after clicking."
        self.description = str(uuid.uuid4())
        i_descr.send_keys(self.description)
        title.click()
        assert (
            i_descr.get_attribute("value") == self.description
        ), "S3-3 The description could not be edited."
        assert i_descr.get_attribute(
            "readonly"
        ), "S3-4 The description should be readonly after losing focus."
        return (
            1,
            "Description changes to editable when clicked, and to read-only when it loses focus.",
        )
    ...
    def step4(self):
        ...
        assert (
            i_name.get_attribute("value") == self.name
        ), "S4-1 The name should be preserved after refresh."
        assert (
            i_aff.get_attribute("value") == self.aff
        ), "S4-2 The affiliation should be preserved after refresh."
        assert (
            i_descr.get_attribute("value") == self.description
        ), "S4-3 The description should be preserved after refresh."
    ...
    def step5(self):
        ...
        assert (
            i_name0.get_attribute("value") == self.name
        ), "S5-1 The name should be preserved after refresh."
        assert (
            i_aff0.get_attribute("value") == self.aff
        ), "S5-2 The affiliation should be preserved after refresh."
        assert (
            i_descr0.get_attribute("value") == self.description
        ), "S5-3 The description should be preserved after refresh."
        ...
        return (
            1,
            "The data is preserved after refresh even when there are multiple items.",
        )
    ...
    def step6(self):
        ...
        for i, c in enumerate(contacts):
            ...
            assert (
                img.get_attribute("src") == test_image_urls[i]
            ), f"S6-1 The image {i} should be there."
        return (
            1,
            "Ihe image can be changed by clicking on the figure tag, and the new image is saved in the database.",
        )
    ...
    def step7(self):
        ...
        i_name = c.find_element(By.CSS_SELECTOR, "input[name='name']")
        assert (
            i_name.get_attribute("value") == self.user_2_name
        ), "S7-4 User 2 should see the new contact."
    ...
    def step8(self):
        ...
        assert (
            len(contacts) == 1
        ), f"S8-2 User 1 should see one contact; it sees {len(contacts)}."
        self.refresh()
        time.sleep(0.5)
        contacts = self.get_contacts()
